...
想象一下，我们有一个系统，它在网页上展示财务数据，可以滚动查看，而负数会用红色标注。 现在，如果有人提出要求，要把这些数据转换成报告形式，用于黑白打印机打印。报告需要有正确的分页，包括页眉、页脚和列标题，而且负数要用括号标出。 显然，这需要编写一些新的代码。但是，一个好的软件架构设计应该使得需要修改的旧代码量降到最少。理想情况下，这个数量应该为零。
